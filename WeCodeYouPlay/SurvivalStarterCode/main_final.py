...
f1 = Font(black, 36, red, "Chiller")
f2 = Font(black, 18, red, "Chiller")
f3 = Font(black, 250, red, "Chiller")

ouch = Sound("sounds/ouch.wav",0)
drink = Sound("sounds/drink.wav",1)
die = Sound("sounds/die.wav",2)

while not game.over:
  game.processInput()
  game.scrollBackground("down",2)
  game.drawText("Survival", 140, 100, f3)
  game.drawText("Press [SPACE] to Start", 250, game.height - 100, f1)
...
while not game.over:
  game.processInput()
  game.scrollBackground("down",2)
  game.drawText("Game Over", 10, 50, f3)
  game.drawText("Press [SPACE] to Exit", 250, game.height - 100, f1)
  if keys.Pressed[K_SPACE]:
    game.over = True
  game.update(30)
